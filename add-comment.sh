...
post_picutre() {
    curl -sSL -H "${AUTH_HEADER}" -H "${API_HEADER}" -d '{"body": "![quick_and_flupke_oups.jpg]('${PICURE_URL}')"}' -H "Content-Type: application/json" -X POST "${URI}/repos/${GITHUB_REPOSITORY}/issues/${NUMBER}/comments"
}
